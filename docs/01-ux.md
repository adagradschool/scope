...
| Key | Action |
|-----|--------|
| `n` | New session (prompts for task, opens as split pane with Claude Code) |
| `enter` | Attach to selected session (full screen) |
...
`scope top` doesn't own the Claude processes—the terminal multiplexer does. `scope top` just reads the `.scope/` directory and renders it.

## Command Reference

| Command | User | Purpose |
|---------|------|---------|
| `scope top` | Human | Interactive control panel |
...
- `scope top` → see everything, attach anywhere, abort when needed
