...
def test_spawn_with_id_creates_alias_file(
    runner, mock_scope_base, cleanup_scope_windows
):
    """Test spawn --id creates alias file."""
    result = runner.invoke(
        main, ["spawn", "--id", "foo", "--checker", "true", "Test task"]
    )
...
def test_spawn_duplicate_alias_rejected(runner, mock_scope_base, cleanup_scope_windows):
    ...
    result1 = runner.invoke(
        main, ["spawn", "--id", "foo", "--checker", "true", "First task"]
    )
    assert result1.exit_code == 0

    # Second spawn with same alias should fail
    result2 = runner.invoke(
        main, ["spawn", "--id", "foo", "--checker", "true", "Second task"]
    )
